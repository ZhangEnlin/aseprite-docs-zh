...
1. 如果标记的矩形接触到现有的切片，那些切片将被
   选中。
1. 你可以拖放一组选定的切片，将它们移动到
   其他位置。或者你可以从角落或边缘拖动来调整整组切片的大小。
1. 选择一些切片后，你可以按 Delete 键或使用 *编辑 > 删除* 菜单选项将其删除。
1. 双击一个切片，你将看到[切片
   属性](#slice-properties)对话框。
...
你可以使用
[--split-slice 选项](cli.md/#split-slices) 将每个切片导出为不同的 Sprite。

你还可以使用 [--data 选项](cli.md/#data) 或 *文件 > 导出 Sprite Sheet*
菜单选项，并勾选 JSON 输出，将切片信息导出到一个 Sprite Sheet JSON 中。以下是导出数据的示例：
