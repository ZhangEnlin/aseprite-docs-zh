...
你可以使用其中一个选区工具来框选 Sprite 的某些部分，例如矩形选框工具 ![Marquee Tool Icon](./tools/marquee-tool.png)（按 <kbd>M</kbd> 键），
...
在[上下文栏](./context-bar.md)中，你可以找到一组修改器，用于更改对所选区域执行的操作：![修改器](./selecting/modifiers.png)

默认情况下，当你按下鼠标左键，拖动它，然后松开时，它将替换整个选区。但你可以使用其他选项修改此行为（每个选项都有一个键盘快捷键）：
...
你可以使用 *选择 > 取消选择* (<kbd>Ctrl + D</kbd> 或 <kbd>⌘ D</kbd>) 隐藏当前选择。然后你可以使用 *选择 > 重选* (<kbd>Ctrl + Shift + D</kbd> 或 <kbd>⇧ ⌘ D</kbd>) 使其再次出现。

## 反选

你可以使用 *选择 > 反选* (<kbd>Ctrl + Shift + I</kbd> 或 <kbd>⇧ ⌘ I</kbd>) 反转选区。
