...
可以通过 *编辑 > [键盘快捷键](./keyboard-shortcuts.md)* 菜单选项或 <kbd>Ctrl + Alt + Shift + K</kbd> 键（在 macOS 上为 <kbd>⌥ ⇧ ⌘ K</kbd>）来自定义键盘快捷键或组合键 + [鼠标滚轮](./mouse-wheel.md)（或[鼠标移动](./drag-value.md)）。

在[快速参考](/quickref/)页面中有默认键盘快捷键的总体概述。

## 菜单和命令

在这些部分中，你将找到一种方法，为 Aseprite 中每个可从[菜单栏](./menu-bar.md)访问的操作/命令，以及其他一些不属于主菜单但仍可供高级用户使用的命令（例如，“设置墨水类型”不与任何菜单关联或具有默认键盘快捷键，但可以从“命令”部分为其分配一个键）分配一个键。
...
* **形状工具**：这些键可以在你使用矩形或椭圆类工具（例如 ![矩形图标](./tools/rectangle-tool.png) 矩形、
  ...
  * **移动原点**：可以按下 <kbd>空格键</kbd> 在松开鼠标按键前将整个形状移动到新位置，以防原点未正确指定。
...
* **平移选区**：当你正在移动选区时。
  ...
  * **锁定轴向**：可以使用 <kbd>Shift</kbd> 键将移动锁定在仅 X 轴或 Y 轴方向上。
...
* **旋转选区**：当你正在旋转选区时。
  * **角度锁定**：可以使用 <kbd>Shift</kbd> 键将角度锁定为 26.6°、45°、90° 等角度。

* **触发鼠标左键/右键**：可以配置一个特定的键，在[精灵编辑器](./sprite-editor.md)中模拟鼠标左键/右键。

## 键盘 + 鼠标

可以配置为当按下某个键并移动鼠标滚轮（或鼠标本身沿特定轴移动）时触发一些特殊操作：
