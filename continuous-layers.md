...
连续图标 ![断开图层图标](./continuous-layers/broken-layer.png) 指示了在复制[ Cel ](./cel.md)时你偏好的类型。它会修改某些命令的行为，例如[新建帧](./new-frame.md)、[复制帧](./copy-frames.md) 或 [复制 Cel](./copy-cels.md)。

有两种可能的状态：

1. ![普通图层图标](./continuous-layers/broken-layer.png) 此图层为普通图层（具有不连续的 Cel ）：新创建的 Cel 将是不链接的（ Cel 本身被复制)。
