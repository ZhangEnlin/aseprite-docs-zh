...
在 Aseprite 的早期版本中，我们最初尝试加载此 Wintab 库，但这可能会导致程序随机崩溃。有时你可以通过重新安装驱动程序、重启 Windows、在[数位板选项](./tablet.md)中使用 Pointer API，或者仅仅通过 *编辑 > 首选项 > 数位板*（或在旧版本中通过 *编辑 > 首选项 > 实验性*）禁用 Wintab 使用来解决这个问题：
...
这将避免加载 `WinTab32.dll` 文件。你的数位板可能无法正常工作，但至少 Aseprite 可以执行并使用你的鼠标/触控板（或者你可以尝试 [Windows Pointer API](./tablet.md)）。

## Steam

在 Steam 上，你可以在 Aseprite 启动选项中添加 `-disable-wintab` 选项：
