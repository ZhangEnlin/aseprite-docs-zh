...
**Tilemap** 是一种特殊的图层，画布上的每个网格单元格都引用其 *tileset* 中的一个 tile（一个小图像）。通过 *图层 > 新建 > 新建Tilemap 图层* 或按 <kbd>Space + N</kbd> 来创建新的 tilemap。

**tileset** 是一组 tile 的集合，就像颜色调色板是一组颜色的集合一样。每个 tile 都有一个索引，并且可以在画布上每个 tilemap 网格单元格中的不同位置重复使用。
